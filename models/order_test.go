...
func testOrderToManyItems(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	queries.Assign(&b.OrderID, a.OrderID)
	queries.Assign(&c.OrderID, a.OrderID)
	...
	slice := OrderSlice{&a}
	if err = a.L.LoadItems(ctx, tx, false, (*[]*Order)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testOrderToManyAddOpItems(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		...
		if !queries.Equal(a.OrderID, first.OrderID) {
			t.Error("foreign key was wrong value", a.OrderID, first.OrderID)
		}
		if !queries.Equal(a.OrderID, second.OrderID) {
...
func testOrderToManySetOpItems(t *testing.T) {
	var err error
	...
	if !queries.Equal(a.OrderID, d.OrderID) {
		t.Error("foreign key was wrong value", a.OrderID, d.OrderID)
	}
	if !queries.Equal(a.OrderID, e.OrderID) {
		t.Error("foreign key was wrong value", a.OrderID, e.OrderID)
	}
...
func testOrderToManyRemoveOpItems(t *testing.T) {
	var err error
...
var (
	orderDBTypes = map[string]string{`OrderID`: `bigint`, `OrderDescription`: `text`, `CustomerID`: `bigint`}
	_            = bytes.MinRead
)
