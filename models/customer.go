...
var CustomerWhere = struct {
	...
	CustomerID:  whereHelperint64{field: "\"simple\".\"customer\".\"customer_id\""},
	Description: whereHelpernull_String{field: "\"simple\".\"customer\".\"description\""},
}
...
var (
	customerAllColumns            = []string{"customer_id", "description"}
	customerColumnsWithoutDefault = []string{"customer_id", "description"}
...
func (o *Customer) Orders(mods ...qm.QueryMod) orderQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"simple\".\"order\".\"customer_id\"=?", o.CustomerID),
	)
	...
	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"simple\".\"order\".*"})
	}
...
func (customerL) LoadOrders(ctx context.Context, e boil.ContextExecutor, singular bool, maybeCustomer interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(qm.From(`simple.order`), qm.WhereIn(`simple.order.customer_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *Customer) SetOrders(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Order) error {
	query := "update \"simple\".\"order\" set \"customer_id\" = null where \"customer_id\" = $1"
...
func Customers(mods ...qm.QueryMod) customerQuery {
	mods = append(mods, qm.From("\"simple\".\"customer\""))
...
func FindCustomer(ctx context.Context, exec boil.ContextExecutor, customerID int64, selectCols ...string) (*Customer, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"simple\".\"customer\" where \"customer_id\"=$1", sel,
	)
...
func (o *Customer) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"simple\".\"customer\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"simple\".\"customer\" %sDEFAULT VALUES%s"
		}
...
func (o *Customer) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"simple\".\"customer\" SET %s WHERE %s",
...
func (o CustomerSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"simple\".\"customer\" SET %s WHERE %s",
...
func (o *Customer) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"simple\".\"customer\"", updateOnConflict, ret, update, conflict, insert)
...
func (o *Customer) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"simple\".\"customer\" WHERE \"customer_id\"=$1"
...
func (o CustomerSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"simple\".\"customer\" WHERE " +
...
func (o *CustomerSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"simple\".\"customer\".* FROM \"simple\".\"customer\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, customerPrimaryKeyColumns, len(*o))
...
func CustomerExists(ctx context.Context, exec boil.ContextExecutor, customerID int64) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"simple\".\"customer\" where \"customer_id\"=$1 limit 1)"
