...
func testItemsExists(t *testing.T) {
	...
	e, err := ItemExists(ctx, tx, o.ItemID)
	if err != nil {
...
func testItemsFind(t *testing.T) {
	...
	itemFound, err := FindItem(ctx, tx, o.ItemID)
	if err != nil {
		t.Error(err)
	}
...
func testItemToOneOrderUsingOrder(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	queries.Assign(&local.OrderID, foreign.OrderID)
	...
	if !queries.Equal(check.OrderID, foreign.OrderID) {
		t.Errorf("want: %v, got %v", foreign.OrderID, check.OrderID)
	}

	slice := ItemSlice{&local}
	if err = local.L.LoadOrder(ctx, tx, false, (*[]*Item)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testItemToOneSetOpOrderUsingOrder(t *testing.T) {
	var err error
	...
	for i, x := range []*Order{&b, &c} {
		...
		if !queries.Equal(a.OrderID, x.OrderID) {
		...
		if !queries.Equal(a.OrderID, x.OrderID) {
			t.Error("foreign key was wrong value", a.OrderID, x.OrderID)
		}
	}
}

func testItemToOneRemoveOpOrderUsingOrder(t *testing.T) {
	var err error
...
var (
	itemDBTypes = map[string]string{`ItemID`: `bigint`, `ItemDescription`: `text`, `OrderID`: `bigint`}
	_           = bytes.MinRead
)
